...
interface DataAnalysisStopsProps {
...
const getSubprocessStats = (
  ...
  const dataPoints = activeLotes.map(lote => {
    const value = getLoteSubprocessStopMs(lote, no_paso);
    return { lote, value };
  });
...
function ProductGroup({
  ...
  const handleExportExcel = () => {
    ...
    groupedSubprocesses.forEach((processGroup) => {
      processGroup.items.forEach((sub) => {
        ...
        activeLotes.forEach((lote) => {
          ...
          const stopMs = dp ? dp.value : 0;
          const stopMinutes = stopMs > 0 ? Number((stopMs / 60000).toFixed(2)) : 0;
          rowCells.push(stopMinutes);
        });
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Análisis Paradas');
    ...
    const fileName = `${productCode}-paradas_${dateString}.xlsx`;
  ...
  const maxStopMsInProductGroup = useMemo(() => {
    let maxVal = 0;
  ...
  const roundedMaxLimit = useMemo(() => {
    return roundToCleanTime(maxStopMsInProductGroup);
  }, [maxStopMsInProductGroup]);
                  ...
                  <tr className="h-12" style={{ height: '48px' }}>
                    ...
                    <th className="px-4 pt-1.5 pb-1 relative bg-slate-50 z-20 align-middle">
                      <div className="text-[10px] font-black text-slate-600 uppercase tracking-wider mb-1 flex justify-between">
                        <span>Tiempo de Parada (Eje de tiempo unificado)</span>
                              ...
                              {activeLotes.map((lote, lIdx) => {
                                ...
                                if (xSpan > 0) {
                                  if (value > xMin) {
                                    ...
                                    barWidth = Math.max(1.5, absoluteRatio * 12);
                                    isCroppedLeft = true;
                                  }
                                }
                                ...
                                let barColorClass = 'bg-red-500 hover:bg-red-400';
                                let strokeColor = '#ef4444';
                                
                                if (isOutsideHigh) {
                                  barColorClass = 'bg-red-800 hover:bg-red-700';
                                  strokeColor = '#991b1b';
                                } else if (isOutsideLow) {
                                  barColorClass = 'bg-red-300 hover:bg-red-200';
                                  strokeColor = '#fca5a5';
                                }
                                  ...
                                  <React.Fragment key={`${sub.no_paso}-${lote}`}>
                                    ...
                                    >
                                      <div className="relative h-full min-h-[48px] w-full flex flex-col justify-center">
                                        ...
                                        <div className="absolute inset-0 z-10 px-4 py-2 w-full">
                                          ...
                                          {hasStats && meanPct > 0 && meanPct < 100 && (
                                            ...
                                            >
                                              {lIdx === 0 && (
                                                <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 bg-rose-50 text-rose-700 text-[8px] font-black font-mono px-1 py-0.5 rounded border border-rose-200 shadow-3xs z-20 whitespace-nowrap uppercase tracking-wider">
                                                  Promedio: <span className="normal-case">{formatDuration(stats.mean)}</span>
                                                </div>
                                              )}
                                            </div>
                                          )}
                                              ...
                                              <div className="group/bar relative h-6 w-full">
                                                ...
                                                <div className="absolute opacity-0 pointer-events-none group-hover/bar:opacity-100 transition-opacity bg-slate-950 text-white text-[10px] p-2.5 rounded-lg shadow-xl z-50 -top-16 left-1/2 -translate-x-1/2 whitespace-nowrap border border-slate-800">
                                                  ...
                                                  <p className="font-bold text-slate-300">
                                                    Tiempo de parada: <span className="text-white font-extrabold">{formatDuration(value)}</span>
                                                  </p>
                                        ...
                                        <div className="absolute inset-y-0 left-4 right-4 pointer-events-none z-40">
                                          <div 
...
export default function DataAnalysisStops({ 
  tareas, 
  registroTiempos, 
  lotesCargados = [],
  exceptions = [],
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">
              Análisis Comparativo de Tiempos de Paradas
            </h2>
          </div>
          <p className="text-xs text-slate-500 font-semibold mt-1">
            Esta sección agrupa automáticamente los lotes según su código de producto y compara estadísticamente el tiempo neto de parada.
          </p>
        </div>
